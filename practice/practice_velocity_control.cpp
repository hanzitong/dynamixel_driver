...
int main(int argc, char* argv[])
{
    ...
    /* write goal velocity */
    dxl_comm_result = packetHandler -> write4ByteTxRx(portHandler, DXL_ID, ADDR_GOAL_VELOCITY, VELOCITY_LIMIT-20, &dxl_error);
    if (dxl_comm_result != 0 || dxl_error != 0) {
        std::cout << "fail to write goal velocity" << std::endl;
    } else {
        std::cout << "succeed to set goal velocity" << std::endl;
    }

    /* check velocity */
    std::cout << "press any key to continue. ESC: exit" << std::endl;
    while (true) {
        if (getch() == ESC_ASCII_VALUE) {
            /* stop motor */
            break;
        } else if (false) {
            /* set new goal velocity */
        } else {
            ...
            if (dxl_comm_result != 0 || dxl_error != 0) {
                std::cout << "fail to read present velocity" << std::endl;
            } else {
                std::cout << "present velocity: " << dxl_present_velocity << "[rev/min]" << std::endl;
            }
        }
    }
